...
function Overview({ data }) { 
    ...
    const metrics = [
        {
            label: 'SAFETY RATING',
            value: data?.safetyRating ?? 'N/A',
            badge: data?.safetyRating?.toUpperCase(),
        ...
        {
            label: 'VEHICLE OOS',
            value: data?.vehicleOos ?? '0.0%',
            badge: 'VS NATL AVG 21%',
    ...
    return (
        <div className="bg-[#f8fafc] p-[32px] space-y-[24px]">
            ...
            {/* TOP ROW: GAUGE AND VERIFICATIONS */}
                    ...
                    <div className="space-y-4">
                        {verifications.map((item, index) => (
                            <div key={index} className="flex items-center justify-between py-1">
                                <span className="text-[14px] font-[600] text-[#334155]">{item.label}</span>
                                {item.isPass ? <CheckCircleOutlined className="text-[#10b981] !text-[24px]" /> : <ErrorOutlined className="text-[#ef4444] !text-[24px]" />}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-4 gap-[16px]">
                {metrics.map((item, index) => (
                    <div key={index} className="bg-white rounded-[12px] border border-[#e5eaf2] p-[20px] shadow-sm relative">
                        <div className="mb-[20px] flex items-center justify-between">
                            <div className={`grid h-[40px] w-[40px] place-items-center rounded-[10px] ${item.iconBg}`}>
                                {React.cloneElement(item.icon, { className: `!text-[22px] ${item.iconColor}` })}
                            </div>
                            {item.badge && <span className={`rounded-[4px] px-[8px] py-[3px] text-[9px] font-[800] uppercase tracking-wide ${item.badgeClass}`}>{item.badge}</span>}
                            {item.hasDot && <div className="absolute top-4 right-4 h-2 w-2 rounded-full bg-[#ef4444]" />}
                        </div>
                        <div className="mb-[4px] text-[10px] font-[700] uppercase tracking-wider text-[#94a3b8]">{item.label}</div>
                        <div className="text-[20px] font-[400] text-[#0f172a]">{item.value}</div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-12 gap-[24px]">
                ...
                <div className="col-span-6 bg-white rounded-[18px] border border-[#d9e1ee] overflow-hidden shadow-sm">
                    ...
                    <div className="p-[24px]">
                        <table className="w-full">
                            ...
                            <tbody>
                                <tr className="border-b border-[#eef2f7]">
                                    <td className="py-[18px] text-[13px] text-[#334155]">Inspections</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.vehicleInspections || '0'}</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.driverInspections || '0'}</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.hazmatInspections || '0'}</td>
                                </tr>
                                <tr className="border-b border-[#eef2f7]">
                                    <td className="py-[18px] text-[13px] text-[#334155]">Out Of Service</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.vehicleOos || '0'}</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.driverOos || '0'}</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.hazmatOos || '0'}</td>
                                </tr>
                                <tr className="border-b border-[#eef2f7]">
                                    <td className="py-[18px] text-[13px] text-[#334155]">OOS %</td>
                                    <td className="text-center"><span className="bg-[#fee2e2] text-[#ef4444] px-2 py-1 rounded-full text-[11px] font-[700]">{inspections?.vehicleOosRate || '0%'}</span></td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.driverOosRate || '0%'}</td>
                                    <td className="text-center text-[13px] text-[#334155]">{inspections?.hazmatOosRate || '0%'}</td>
                                </tr>
                                <tr>
                                    <td className="py-[18px] text-[13px] text-[#334155]">OOS % Nat'l Avg</td>
                                    <td className="text-center text-[13px] text-[#64748b]">{inspections?.nationalAvgVehicle || '0%'}</td>
                                    <td className="text-center text-[13px] text-[#64748b]">{inspections?.nationalAvgDriver || '0%'}</td>
                                    <td className="text-center text-[13px] text-[#64748b]">{inspections?.nationalAvgHazmat || '0%'}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div className="col-span-6 bg-white rounded-[18px] border border-[#d9e1ee] overflow-hidden shadow-sm">
                    ...
                    <div className="p-[24px] grid grid-cols-2 gap-[16px]">
                        {[
                            { label: 'Fatal', value: crashes?.fatal || '0', subText: '— No change' },
                            { label: 'Injury', value: crashes?.injury || '0', subText: '+1 vs prev.' },
                            { label: 'Tow', value: crashes?.tow || '0', subText: '-2 vs prev.' },
                            { label: 'Total Crashes', value: crashes?.total || '0', subText: '-8% MoM', danger: true }
                        ].map((item, index) => (
                    ...
                    <div className="flex items-center justify-between px-[24px] py-[20px] border-t border-[#eef2f7]">
                        <div>
                            ...
                            <p className="mt-2 text-[15px] font-[700] text-[#0f172a]">{crashes?.lastCrashDate || '--'}</p>
                        </div>
